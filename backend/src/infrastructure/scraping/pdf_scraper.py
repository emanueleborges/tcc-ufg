...
class PdfScraper:
    ...
    def run(self, on_progress: Callable[[int, str], None] | None = None) -> int:
        ...
        candidates = self._search_candidates(deadline, on_progress=report)
        ...
        report(100, f"Concluído. Total na base: {len(saved)}")
        return len(saved)
        ...
        *,
        ...
        for index, candidate in enumerate(tqdm(expanded, desc="Baixando e filtrando")):
            ...
            if on_progress:
                ...
                on_progress(
                    percent,
                    f"Baixando PDFs ({index + 1}/{total}) · aceitos: {new_saved_count}",
                )
